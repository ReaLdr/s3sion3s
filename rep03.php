...
header("Content-Disposition: attachment; filename=Reportes de intervenciones relevantes.xls");
...
//echo $de
	...
	$accion="GeneraReporte Intervenciones";
	bitacora($accion);
...
echo "<font style='font-size:16px;font-weight:bold;'>REPORTE DE INTERVENCIONES RELEVANTES<br></font><br>";
echo "</th>";
echo "</tr>";
echo "<tr>";
echo "<th colspan=7>";
...
echo "<th class='borde_tabla'>Punto del orden del d&iacute;a en que se present&oacute; la intervenci&oacute;n</th>";
echo "<th class='borde_tabla'>Nombre Participante (s)</th>";
echo "<th class='borde_tabla'>Cargo </th>";
echo "<th class='borde_tabla'>Punto de Intevencion </th>";
echo "<th class='borde_tabla'>Descripci&oacute;n breve de la intervenci&oacute;n</th>";
...
$sql_consulta = "SELECT * FROM sisesecd_sesiones WHERE id_sesion=$id_sesion and id_distrito = $id_distrito AND estatus=1 ORDER BY sisesecd_sesiones.id_distrito ASC";
...
//echo $sql_consulta1;
	$intervenciones_consulta1 = sqlsrv_query($conn, $sql_consulta1);
		...
		$sql_consultaCAT ="SELECT id_integrante, tipo_acredor, nombre, ap_paterno, ap_materno FROM sisesecd_cat_funcionarios WHERE id_sesion=$idsesion and id_distrito =$iddistrito order by id_integrante";
	...
	echo "<td>"	.$rowsesion['id_distrito']."</td>";
	$idsesion=$rowsesion['id_sesion'];
	echo "<td></td><td></td><td></td><td>Sin intervenciones relevantes</td><td></td>";
	}
	echo "</tr>";
