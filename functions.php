...
function validasesion(){
	...
	echo "Debe seleccionar un numero de sesion";
...
function calendar_html($campo){
	...
	<table style="width:200px;text-align:center;border:1px solid #808080;border-bottom:0px;" cellpadding="0" cellspacing="0">
	 <tr>
	  <td colspan="4">
	  	<select id="calendar_mes" onchange="update_calendar(<?php echo "'".$campo."'"?>)">
		 <?php
		 $mes_numero=1;
		 while($mes_numero<=12){
		 	if($mes_numero==$mes){
	  ...
	  <td colspan="3">
	  	<select style="width:70px;" id="calendar_anio" onchange="update_calendar(<?php echo "'".$campo."'"?>)">
		 <?php
		 // años a mostrar
	...
	<?php calendar($mes,$anio,$campo) ?>
	</div>
	<?php
}

function calendar($mes,$anio,$campo){
	$dia=1;
	...
	while($j<$total_dias){
		...
		while($i<7){
			if($j<=$numero_primer_dia){
				...
				//echo " <td><a style=\"display:block;cursor:pointer;\" onclick=\"set_date('".$diames_con_cero."-".$mes."-".$anio."','".$campo."')\">".$diames."</a></td> \n";
				echo " <td><a style=\"display:block;cursor:pointer;\" onclick=\"set_date('".$anio."-".$mes."-".$diames_con_cero."','".$campo."')\">".$diames."</a></td> \n";
				$diames++;
			}
			$i++;
			$j++;
