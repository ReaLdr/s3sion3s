...
$sql_inicio="select s.id_sesion,s.id_distrito,nosesion,desc_sesion,tipo_sesion, fecha_inicio_prog,hora_inicio_prog,con_inicio,con_orden,con_votos,con_intervencion,con_incidente, con_fin,estatus,id_inicio,i.id_sesion,fecha_inicio_real,hora_inicio_real, qi_cp,qi_c1,qi_c2,qi_c3,qi_c4,qi_c5,qi_c6,qi_se,qi_pan_p,qi_pan_s,qi_pri_p,qi_pri_s,qi_prd_p,qi_prd_s, qi_pt_p,qi_pt_s,qi_pvem_p,qi_pvem_s,qi_pmc_p, qi_pmc_s, qi_elg_p, qi_elg_s, qi_pes_p, qi_pes_s, qi_prsp_p, qi_prsp_s, qi_pfsm_p, qi_pfsm_s, qi_morena_p, qi_morena_s, qi_ci1_p, qi_ci1_s, qi_ci2_p, qi_ci2_s, qi_ci3_p, qi_ci3_s, qi_prensa, qi_radio, qi_tv, CAST(observaini as CHAR(2084))as observaini,quorumini,asistencia FROM sisesecd_sesiones as s,sisesecd_inicio as i WHERE s.id_sesion=i.id_sesion
...
//echo "sql_inicio: " . $sql_inicio;
$head=sqlsrv_query($conn, $sql_inicio);
$undato_1 = sqlsrv_fetch_array ($head);
if($undato_1){
	$fecha_sesion= "Sesión celebrada el: " . $undato_1['fecha_inicio_real'];
} else{
	$fecha_sesion = "No se encontró sesión alguna";
}

//(is_null($undato_1['fecha_inicio_real']) ? $fecha_sesion = "No se encontró sesión alguna" : $fecha_sesion=$undato_1['fecha_inicio_real']);
...
$reanudada=0;
$prolongada=0;
//$fe=date_create($fecha); //comented by Rea
//$f1=date_format($fe, 'Y-m-d');
//$f=date_format($fe, 'd-m-Y'); //comented by Rea
//echo $f;

$sql_count_estado="select count(*) as iniciadas from sisesecd_inicio where id_sesion in( select id_sesion from sisesecd_sesiones where nosesion=$nosesion and tipo_sesion=$typesess and desc_sesion=$descsesion and id_distrito!=40 and estatus=1)";
//echo "sql_count_estado: " . $sql_count_estado;//INICIADAS
$r_iniciadas=sqlsrv_query($conn, $sql_count_estado);
$d_iniciadas=sqlsrv_fetch_array($r_iniciadas);
...
$sql_count_estado2="select count(*) as cuantas from sisesecd_estado_sesion where estado_sesion=2 and id_sesion in (
	...
	and id_distrito!=40
	and estatus=1)";
	//echo "sql_count_estado2: " . $sql_count_estado2; //CUANTAS O SIN QUORUM
	$r_sinq=sqlsrv_query($conn, $sql_count_estado2);
	$d_sinq=sqlsrv_fetch_array($r_sinq);
	...
	(
		...
		and id_distrito!=40
		and estatus=1)";
		//echo "sql_count_estado3: " . $sql_count_estado3; //CUANTAS2 O SEGUNDA
		$r_dos=sqlsrv_query($conn, $sql_count_estado3);
		$d_dos=sqlsrv_fetch_array($r_dos);
		...
		$sql_count_estado4="select count(*) as cuantas3 from sisesecd_fin where id_sesion in (
			...
			and id_distrito!=40
			and estatus=1)";
			//echo "sql_count_estado4: " . $sql_count_estado4; //CUANTAS3 O concluida
			$r_conc=sqlsrv_query($conn, $sql_count_estado4);
			$d_conc=sqlsrv_fetch_array($r_conc);
			...
			$sql_count_estado5="select count(*) as cuantas4 from sisesecd_estado_sesion where estado_sesion=5 and id_sesion in (
				...
				and id_distrito!=40
				and estatus=1)";
				//echo "sql_count_estado5: " . $sql_count_estado5; //CUANTAS4 O RECESO
				$r_receso=sqlsrv_query($conn, $sql_count_estado5);
				$d_receso=sqlsrv_fetch_array($r_receso);
				...
				$sql_count_estado6="select count(*) as cuantas5 from sisesecd_estado_sesion where estado_sesion=6 and id_sesion in (
					...
					and id_distrito!=40
					and estatus=1)";
					//echo "sql_count_estado6: " . $sql_count_estado6; //CUANTAS5 O CONC_RECESO
					$r_rc=sqlsrv_query($conn,$sql_count_estado6);
					$d_rc=sqlsrv_fetch_array($r_rc);
					...
					and id_distrito!=40
					and estatus=1)";
					$r_suspendida=sqlsrv_query($conn, $sql_count_estado7);
					...
					and id_distrito!=40
					and estatus=1)";
					$r_reanudada=sqlsrv_query($conn, $sql_count_estado8);
					...
					and id_distrito!=40
					and estatus=1)";
					$r_prol=sqlsrv_query($conn, $sql_count_estado9);
					$d_prol=sqlsrv_fetch_array($r_prol);
					...
					<body onLoad="javascript:refresca()">
						...
						<div id="generales">
							<?php
							echo "<h3>REPORTE DE SEGUIMIENTO A LA SESION</h3><hr>";
							...
							while($datos = sqlsrv_fetch_array ($r_inicio))
							{
								...
								if($datos['con_fin']==1){
									$sql_fin="SELECT f.id_fin, f.id_sesion, f.fecha_inicio_final, f.hora_fin_final, f.qf_cp, f.qf_c1, f.qf_c2, f.qf_c3, f.qf_c4, f.qf_c5, f.qf_c6, f.qf_se, f.qf_pan_p, f.qf_pan_s, f.qf_pri_p, f.qf_pri_s, f.qf_prd_p, f.qf_prd_s, f.qf_pt_p, f.qf_pt_s, f.qf_pvem_p, f.qf_pvem_s, f.qf_pmc_p, f.qf_pmc_s, f.qf_elg_p, f.qf_elg_s, f.qf_pes_p, f.qf_pes_s, f.qf_prsp_p, f.qf_prsp_s,  qf_pfsm_p, qf_pfsm_s, f.qf_morena_p, f.qf_morena_s, f.qf_ci1_p, f.qf_ci1_s, f.qf_ci2_p, f.qf_ci2_s, f.qf_ci3_p, f.qf_ci3_s, f.qf_prensa, f.qf_radio, f.qf_tv, quorumfin, CAST(f.observafin as CHAR(2048))as observafin from sisesecd_fin as f where id_sesion in( select id_sesion from sisesecd_sesiones where nosesion=$nosesion and tipo_sesion=$typesess and desc_sesion=$descsesion and id_distrito=$datos[id_distrito] and estatus=1);";
								...
								if($datos['con_incidente']==1){
									$sql_incidentes="select TOP(1) s.id_sesion, s.id_distrito, s.nosesion, s.desc_sesion, s.tipo_sesion, s.fecha_inicio_prog, s.hora_inicio_prog, s.con_inicio, s.con_orden, s.con_votos, s.con_intervencion, s.con_incidente, s.con_fin, s.estatus, CAST(inci.incidente as CHAR(2084)) as incidente from sisesecd_sesiones as s, sisesecd_incidentes as inci WHERE  s.id_sesion=inci.id_sesion and s.id_sesion  in( select id_sesion from sisesecd_sesiones where nosesion=$nosesion and tipo_sesion=$typesess and desc_sesion=$descsesion and id_distrito=".$datos['id_distrito']." And estatus=1) order by inci.id_incidentes desc;";
									//echo "sql_incidentes: " . $sql_incidentes;
								...
								if($datos['con_intervencion']==1){
									$sql_participaciones ="SELECT TOP(1) s.id_sesion, s.id_distrito, s.nosesion, s.desc_sesion, s.tipo_sesion, s.fecha_inicio_prog, s.hora_inicio_prog, s.con_inicio, s.con_orden, s.con_votos, s.con_intervencion, s.con_incidente, s.con_fin, s.estatus, CAST(inter.intervencion as CHAR(2084)) as intervencion, inter.punto FROM sisesecd_sesiones as s, sisesecd_intervenciones as inter	WHERE s.id_sesion=inter.id_sesion and s.id_sesion in( select id_sesion from sisesecd_sesiones where nosesion=$nosesion and tipo_sesion=$typesess and desc_sesion=$descsesion and id_distrito=".$datos['id_distrito']." and estatus=1) order by inter.id_intervencion desc;";
									...
									//echo "sql_participaciones: " . $sql_participaciones . "<br>";
								...
								$sql_estado="SELECT TOP(1) * FROM sisesecd_estado_sesion WHERE id_sesion IN ( SELECT id_sesion FROM sisesecd_sesiones WHERE nosesion=$nosesion AND tipo_sesion=$typesess AND desc_sesion=$descsesion AND id_distrito=$datos[id_distrito] AND estatus=1) GROUP BY id_estado,id_sesion,id_distrito, estado_sesion,descripcion,hora_inicio,hora_termino,fecha_alta ORDER BY id_estado DESC;";
								//$sql_estado="select FIRST 1 * from sisesecd_estado_sesion where id_sesion in ( select id_sesion from sisesecd_sesiones where nosesion=$nosesion and tipo_sesion=$typesess and desc_sesion=$descsesion and id_distrito=$datos[id_distrito] and estatus=1) group by id_estado,id_sesion,id_distrito, estado_sesion,descripcion,hora_inicio,hora_termino,fecha_alta order by id_estado desc;";
								//echo "sql_estado: " . $sql_estado;
						...
						<div id="slides">
							<div class="slides_container">
								...
								$i=0;
								$dots= sqlsrv_fetch_array ($r_dots);
								if($dots){
								...
								for ($ip=0;$ip<$size;$ip++){
									...
									qi_prd_p, qi_prd_s, qi_pt_p, qi_pt_s, qi_pvem_p, qi_pvem_s, qi_pmc_p, qi_pmc_s, qi_elg_p, qi_elg_s, qi_pes_p, qi_pes_s, qi_prsp_p, qi_prsp_s, qi_pfsm_p,qi_pfsm_s, qi_morena_p, qi_morena_s, qi_ci1_p, qi_ci1_s, qi_ci2_p, qi_ci2_s, qi_ci3_p, qi_ci3_s, qi_prensa, qi_radio, qi_tv, observaini
									...
									and s.id_sesion in( select id_sesion from sisesecd_sesiones where nosesion=".$nosesion."
										...
										and s.id_sesion in(
											...
											<div>
												<?php
												echo "<h3>SENTIDO DE LA VOTACION&nbsp;&nbsp;&nbsp;&nbsp;PUNTO:".$pto."</h3><hr><p class='resultados'>".utf8_encode($descripcion)."</p>";
												...
												if($cuantos1>=1){
													...
													echo '<td colspan=2  class="borde_tabla">'.number_format((33-$iniciada),0).'</td>';
													echo "</tr>";
													echo "<tr>";
													echo "<td width=100></td>";
													echo "<td width=100></td>";
													...
													echo '<td colspan=2  class="borde_tabla">'.number_format((33-$concluida),0).'</td>';
													echo "</tr>";
													echo "<tr>";
												}else{
